function calcgraphRho(basename,conntype,mattype,varargin)
...
if strcmp(param.randomise,'on')
    ...
    savename = fullfile(filepath,conntype,[basename conntype '_randgraph.mat']);
...
elseif strcmp(param.latticise,'on')
    ...
    savename = fullfile(filepath,conntype,[basename conntype '_latgraph.mat']);
    numruns = param.numrand;
else
    %savename = sprintf('%s/%s/%s%sgraph.mat',filepath,conntype,basename,conntype);
    savename = fullfile(filepath,conntype,[basename conntype '_graph.mat']);
    numruns = 1;
end
...
graphdata{8,1} = 'connection density';
%graphdata{9,1} = 'mutual information';
%graphdata{10,1} = 'modular span';

fprintf('Processing graph metrics %s',basename);

load([filepath filesep conntype filesep basename conntype '.mat']); % contains matrix and bootmat
if ~isempty(mattype)
...
%for f = 1:size(matrix,1)
    ...
    for iter = 1:numruns % for each randomised matrix
        ...
        for thresh = 1:length(tvals)
            if strcmp(param.randomise,'on')
                %randomisation
                if ~isempty(bootmat)
                    cohmat = squeeze(bootmat(:,:,iter));
                else 
                    cohmat=[];
                end
            else
                cohmat = squeeze(matrix(:,:));
            end
    ...
    for iter = 1:numruns
        for thresh = 1:length(tvals)
            ...
            graphdata{1,2}(thresh,1:size(matrix,2),iter) = allcc{iter}(thresh,:);
            
            %characteristic path length
            graphdata{2,2}(thresh,iter) = allcp{iter}(thresh);
            
            %global efficiency
            graphdata{3,2}(thresh,iter) = alleff{iter}(thresh);
            
            % modularity
            graphdata{4,2}(thresh,iter) = mean(allQ{iter}(thresh,:));
            
            % community structure
            graphdata{5,2}(thresh,1:size(matrix,2),iter) = squeeze(allCi{iter}(thresh,1,:));
            
            %betweenness centrality
            graphdata{6,2}(thresh,1:size(matrix,2),iter) = allbet{iter}(thresh,:);
            
            %participation coefficient
            graphdata{7,2}(thresh,1:size(matrix,2),iter) = mean(squeeze(allpc{iter}(thresh,:,:)));
            
            %connection density
            graphdata{8,2}(thresh,iter) = allden{iter}(thresh);
            
            %modular span
            %graphdata{10,2}(thresh,iter) = mean(allms{iter}(thresh,:));
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
