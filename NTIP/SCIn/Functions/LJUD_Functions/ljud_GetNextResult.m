% Call the ljud_GetNextResult function from the MATLAB command window or any other
% mfile. To call the function use the following notation:
%
% [Error IOType Channel ljValue x1 ljUserData] = ljud_GetNextResult(Parameters)
%
...
% information on the GetNextResult function and the required paramters. Calling either
% Go function creates a list of results that matches the list of requests. Use
% GetNextResult function to step through the list of results in order

function [ljError IOType Channel ljValue x1 ljUserData] = ljud_GetNextResult(ljHandle, IOType, Channel, ljValue, x1, ljUserData)
[ljError IOType Channel ljValue x1 ljUserData] = calllib('labjackud','GetNextResult',ljHandle,IOType,Channel,ljValue,x1,ljUserData);
