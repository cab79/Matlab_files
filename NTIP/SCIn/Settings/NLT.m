...
switch opt
    ...
    % settings options
    h.SettingsOptions = {'Ascend','Adapt','NLT_oddball','NLT_simple','Complex','NLT_complex','NLT_complex_entrain'};
    ...
    h.Settings.npulses_train = 2; % set to zero to be determined by stimdur
    % Tactile: within-train frequency (Hz)
    h.Settings.p_freq=2.5;
...
case 'Adapt'
    ...
    h.Settings.adaptive.startinglevel = 2; % for intensity, in dB (e.g. 10); for pitch, in Hz (e.g. 100); for tactile in mA
    ...
    % number of minimum sets to randomise together
    h.Settings.n_set = [1 1 1]; % 1 = use min set size, for each CP condition
    % min number of oddballs within each CP condition
    h.Settings.n_odd = [20, 20, 20]; % overrides h.Settings.totdur
    % min number of oddballs per randomised set, per CP
    h.Settings.n_odd_set = [5, 5, 5]; % overrides h.Settings.totdur
    ...
    h.Settings.standardind = 1; % does not apply to 'roving oddball' design
    % keep oddball trials apart by at least sep_odd standards
    h.Settings.sep_odd = [2 1 2]; % for each CP condition
    % for each set, ensure a number of leading standards 
    h.Settings.std_lead = [0 0 0]; % for each CP condition
    % number of minimum sets to randomise together
    h.Settings.n_set = [1 1 1]; % 1 = use min set size, for each CP condition
    ...
    h.Settings.inten = []; % value between 2 and 1000mA for Digitimer DS8R
    % Tactile: number of pulses in a train
    h.Settings.npulses_train = 10; % set to zero to be determined by stimdur
    % Tactile: within-train frequency (Hz)
