function [pos_ans, q] = BackwardMasking_Pest(std_level, var_level, nAFC)

% A 20-ms, 1-kHz pure tone (the signal) is presented immediately before
% (i.e., no silent gap) a band of bandpass noise of 300-ms (400-1600 Hz).
% All sounds are onset and offset gated by means of two raised cosine onset
% and offset ramps of 10-ms. The subject has to detect the tone (in yes/no
% task) or to tell which interval has the tone.
% - PARAMETER VARIED ADAPTIVELY: the level of the signal;
% - STANDARD LEVEL: the parameter has no use in the current experiment.
...
lf = 400; % lowest frequency of thee noise
hf = 1600; % highest frequency of the noise
...
variable = ConcatenateSounds(signal, noise);
standard = noise;
% [2] %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if ~nAFC
    ...
    q = ['In which interval was the signal? (' num2str(1:nAFC) ') ']; ;
end;