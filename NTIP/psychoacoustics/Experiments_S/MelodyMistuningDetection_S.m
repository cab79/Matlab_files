function [pos_ans, q] = MelodyMistuningDetection_S(std_level, var_level, nAFC)
...
% threshold in cents.
% - PARAMETER VARIED ADAPTIVELY: the delta: the deviation, in cents,
% of the "sol" note from the equitempered tuning (i.e., 700 cents);
% - STANDARD LEVEL: in the current experiment it is set to 700 cents, i.e,
% the equitempred sol.
...
f_sol = f_do_start * 2^((std_level)/1200);
...
f_sol_var = f_do_start * 2^((std_level+var_level)/1200);
