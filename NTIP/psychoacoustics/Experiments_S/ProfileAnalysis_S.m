function [pos_ans, q] = ProfileAnalysis_S(std_level, var_level, nAFC)
...
% given in dB.
% - PARAMETER VARIED ADAPTIVELY: the delta: the difference the level 
% between the level of the components and the level of the target 3rd
% component;
...
var_amps = [10^(std_level/20), ...
    10^(std_level/20), ...
    10^((std_level+var_level)/20), ...
    10^(std_level/20), ...
    10^(std_level/20)];
