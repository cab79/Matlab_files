...
for gi = 1:length(g)
    ...
    g(gi, 1).legend_axe_handle.Position(1) = minlegpos(1);
...
for gi = 1:length(g)
    ...
    g(gi, 1).legend_axe_handle.Position(2) = g(gi, 1).facet_axes_handles.Position(2)+y_extent*0.1;
    g(gi, 1).legend_axe_handle.Title.Position(2) = g(gi, 1).legend_axe_handle.Title.Position(2)*0.1;
end
