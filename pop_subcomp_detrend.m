% cab: ADDS A DETREND FUNCTION - REQUIRES LIST OF iCs TO DETREND
...
function [EEG, com] = pop_subcomp( EEG, components, plotag, detrendcomp )
...
icaact = eeg_getdatact(EEG, 'component', component_keep);
ic2detrend = find(detrendcomp(component_keep));
for i = 1:EEG.trials, icaact(ic2detrend,:,i) = detrend(icaact(ic2detrend,:,i)')'; end;
icaact = reshape(icaact, size(icaact,1), EEG.pnts * EEG.trials);
compproj = EEG.icawinv(:, component_keep)*icaact;
