...
elseif strcmp(subject, 'OA7') == 1 
    [XMAX,IMAX,XMIN,IMIN] = extrema(Cz(:,2000:2200));%%search for extrema on internet - peak detection prog, xmax ampli imax latency
    LAT = IMAX(1)+1999;
    LATn = IMIN(1)+1999;
end

[XMAX,IMAX,XMIN,IMIN] = extrema(mdata1.avg(15,LAT-60:LAT+8));%%each cond seperatly +/-64 ms 15 is CZ
LAT1 = IMAX(1);
...
n21=mdata1.avg(:,(LATn-61+LATn1)-5:(LATn-61+LATn1)+5)';%n2 for condtion 1 
n22=mdata2.avg(:,(LATn-61+LATn2)-5:(LATn-61+LATn2)+5)';%n2 for condition 2
n23=mdata3.avg(:,(LATn-61+LATn3)-5:(LATn-61+LATn3)+5)';
n24=mdata4.avg(:,(LATn-61+LATn4)-5:(LATn-61+LATn4)+5)';
n25=mdata5.avg(:,(LATn-61+LATn5)-5:(LATn-61+LATn5)+5)';
n26=mdata6.avg(:,(LATn-61+LATn6)-5:(LATn-61+LATn6)+5)';
n21_av=mean(n21);
n22_av=mean(n22);
n23_av=mean(n23);
n24_av=mean(n24);
...
%AMPSPN = reshape(AMP(:,:,1:4,:),Nsub,216);
%AMPLEP = reshape(AMP(:,:,4:5,1),Nsub,12);
%save amplitudes3.mat AMPSPN
