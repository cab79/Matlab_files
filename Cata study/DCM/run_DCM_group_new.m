...
if S.loadorsave && exist(fullfile(S.outpath,[sname '.mat']),'file')
    ...
    elseif any(DCM.B{1})
        ...
        Bi = find(double(~(Bv==0)) .* double(Bv>1)); % B indices
    ...
    Ai = find(double(~(Av==0)) .* double(Av>1)); % A indices
    Av(Ai)
    ...
    if sum(DCM.B{:}(:))==0 && nA>1
        ...
        for i = 1:(2^nA);
            ...
            for c = DCM.Ac
                ...
                for f = 1:nA
                    try
                        if k(i,f)
                            A{i}{c} = A{i}{c} + double(DCM.A{c}==Av(Ai(f))) + double(DCM.A{c}==1); 
        ...
        A{1} = sparse(Nareas,Nareas);
        A{2} = sparse(Nareas,Nareas);
        A{3} = sparse(Nareas,Nareas);
        nA=1;
        Afm = A;
    end

    Cv = unique(DCM.C); % c values
    Ci = find(double(~(Cv==0)) .* double(Cv>1)); % c indices
    ...
    if nC>1
        ...
        for i = 1:(2^nC);
            ...
            for f = 1:nC
                try
                    if k(i,f)
                        C{i} = C{i} + double(DCM.C==Cv(Ci(f)))' + double(DCM.C==1)'; 
