...
D.subdirpref = '_spm12_C'; % generic prefix for the SPM file type
...
D.factors = {'Exp'}; % 
D.factortype = {'w'}; % 
D.anatype = 'singlesubject'; % 
%D.TrainTest = {[2 2],[1 1],[]}; % select levels: model trains on first level and tests on second.
...
D.interactions = [0]; % one column per factor; one row per interaction
D.maineffects = [1]; % one column per factor 
