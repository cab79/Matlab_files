...
D.batch_path = 'C:\Data\Matlab\Matlab_files\Cata study\SPManalysis\Source\Design_batch_source_induced_Grp.m';
...
D.data_path = 'C:\Data\Catastrophising study\SPMdata\sourceimages_GS_1grp_noHan_induced';
...
for di = 1:length(identifier)
    ...
    %D.cond_list =  [1 2 1 2 1 2 1 2]';
    D.cond_list =  [1 1 1 1]';
    % factors and statistical model
    %D.factors = {'Grp', 'Exp', 'Subject'}; % must include a subject factor at the end
    %D.factortype = {'g','w','s'}; % w = within, s = subject, g = subject group
    D.factors = {'Grp', 'Subject'}; % must include a subject factor at the end
    D.factortype = {'g','s'}; % w = within, s = subject, g = subject group
