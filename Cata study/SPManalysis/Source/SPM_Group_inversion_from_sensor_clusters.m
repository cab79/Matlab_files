...
S.timewin = [-3000 0]; % Best to narrow this as much as possible to the range of interest. Empty will include whole epoch. 
...
% name of sensor analysis cluster directories to use
S.clusterdir = {'Exp_clusters','Grp_clusters','Grp_Exp_clusters'};
...
for c = 1:length(S.clusterdir)
    ...
    for n = 1:nclus
        ro = size(S.images_out,1);
        S.images_out{ro+1,1} = cell2mat(clustable(nhead+n,11:12));
        S.images_out{ro+1,2} = [];
        S.images_out{ro+1,3} = S.clusterdir{c};
    end
end
image_win = S.images_out;
save(fullfile(S.sensorpath,'image_win.mat'),'image_win');
S.images_out = {S.images_out}
