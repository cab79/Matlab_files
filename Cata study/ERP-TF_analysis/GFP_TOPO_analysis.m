...
avcond = {[1],[2],[3],[4],[5],[6],[7],[8]}; avcondname = 'eachcond'; % all conditions separately
...
% normalise by common average, baseline correct, and normalise by GFP mean over time, for each frequency
for g = 1:length(subjects)
    for f = 1:size(gavg{g,1},3)
        for e = 1:size(gavg{g,1},1)
            gavg{g,1}(e,:,f) = gavg{g,1}(e,:,f) - nanmean(gavg{g,1}(:,:,f),1);
            gavg{g,1}(e,:,f) = gavg{g,1}(e,:,f) - nanmean(gavg{g,1}(e,baseline(1):baseline(2),f),2);
        end
