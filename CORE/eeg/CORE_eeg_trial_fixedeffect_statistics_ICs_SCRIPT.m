...
% recon: all ICs reconstructed into one set of channels; channels considered in a single multivariate analysis
% comp: each IC timecourse considered in separate analyses
...
S.tfce_on = 1; % analyses clusters over time on the GFP
...
if S.tfce_on
    for i=1:size(stats.tfce,3)
        ...
        for c = 1:size(stats.tfce,2)
            c
            significant = find(stats.tfce(:,c,i)<=0.05)
