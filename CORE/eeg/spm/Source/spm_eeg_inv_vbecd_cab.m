function D = spm_eeg_inv_vbecd_cab(D,S)
% CAB version of the SPM12 GUI function for Bayesian ECD inversion (spm_eeg_inv_vbecd_gui.m)
...
% CAB
if exist('S','var') && isfield(S,'timewin') && ~isempty(S.timewin)
    tb=S.timewin;
    ask_tb=0;
else
    ask_tb = 1;
...
if D.ntrials>1
    % CAB
    tr_q = 1;
    if exist('S','var') && isfield(S,'cond') && ~isempty(S.cond)
        if iscell(S.cond{1})
            S.cond = horzcat(S.cond{:});
        end
        ltr = find(ismember(D.conditions,S.cond));
    else
...
while adding_dips
    ...
    if exist('S','var') && isfield(S,'prior') && ~isempty(S.prior) %CAB 
        if length(S.prior)>dip_q
            a_dip = S.prior(dip_q+1).sym;
        else
            a_dip = 0;
        end
    else
    ...
    % add a single dipole to the model
        ...
        % Location prior
        if exist('S','var') && isfield(S,'prior') && ~isempty(S.prior) %CAB 
            spr_q = ~isempty(S.prior(dip_q).prior_loc);
        else
        ...
        if spr_q
            ...
            while 1
                ...
                if exist('S','var') && isfield(S,'prior') && ~isempty(S.prior) %CAB 
                     diags_s0_mni = S.prior(dip_q).prior_var';
                else
        ...
        % Moment prior
        if exist('S','var') && isfield(S,'prior') && ~isempty(S.prior) %CAB 
            wpr_q =  ~isempty(S.prior(dip_q).prior_mom);
        else
        ...
        if wpr_q
            ...
            if exist('S','var') && isfield(S,'prior') && ~isempty(S.prior) %CAB 
                w0_mni = S.prior(dip_q).prior_mom';
                diags_w0_mni = S.prior(dip_q).prior_momvar';
            else
    ...
    % add a pair of symmetric dipoles to the model
        ...
        if spr_q
            ...
            while 1
                ...
                if exist('S','var') && isfield(S,'prior') && ~isempty(S.prior) %CAB 
                    tmp_diags_s0_mni = S.prior(dip_q).prior_var';
                else
                    str2='Prior location variance (mm2)';
        ...
        if wpr_q
            ...
            if exist('S','var') && isfield(S,'prior') && ~isempty(S.prior) %CAB 
                diags_w0 = S.prior(dip_q).prior_momvar';
            else
                str2='Prior moment variance (nAm2)';
...
for ii=1:length(ltr)
 ...
 %% set up figures 
    ...
    inverse.pov(ii) = pov(maxind); % percent of variance explained
    ...
    inverse.jmni{ii} = mni_w;%reshape(mni_w,1,prod(size(mni_w)))'; % dipole(s) orient/ampl in mni space
