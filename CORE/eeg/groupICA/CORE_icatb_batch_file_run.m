...
for nFile = 1:length(inputFiles)
    ...
    %% move output files
    filetypes = {'ica_c','ica_br','pca_r1-','fileinfo_'};
    fileend = {'-1','','',''};
    pref = {[sesInfo.userInput.prefix '_'],[sesInfo.userInput.prefix '_'],[sesInfo.userInput.prefix '_'],''}
    for f = 1:length(filetypes)
        try
            for nf = 1:length(sesInfo.userInput.files)
                indir = fileparts(sesInfo.userInput.files(nf).name);
                fname = fullfile(sesInfo.userInput.pwd,[pref{f} filetypes{f} num2str(nf) fileend{f} '.mat']);
                movefile(fname,[indir '_grp-' filetypes{f} '.mat']);
            end
        end
    end
