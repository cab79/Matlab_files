function CORE_icatb_eeg_import(erpFiles, baseLine, smoothTrials, moving_average_window, sorting_trials, sort_idx, outFileName, outputDir, dsample, n_trials)
% CAB: based on original function icatb_eeg_smooth_trials
...
%     disp(['Loading Subject ', currentErpFile]);
% 
% 
%     % Use EEGLAB function to load set file
%     EEG = icatb_eeg_pop_loadset(currentErpFile); % load .set/.dat file
% %     EEG.event=EEG.event(strcmp({EEG.event(:).type},'DIN2'));
% %     EEG.urevent=EEG.urevent(strcmp({EEG.urevent(:).type},'DIN2'));
%     
% end
...
for nSub = 1:numSubjects
    ...
    % Use EEGLAB function to load set file
    EEG = icatb_eeg_pop_loadset(currentErpFile); % load .set/.dat file
    [mark.conds, mark.tnums, mark.fnums, mark.bnums] = get_markers(EEG);
    
    if ~isempty(dsample)
        EEG = pop_resample(EEG,dsample);
    end
    
%     fprintf('\n');
%     currentErpFile = deblank(erpFiles(nSub, :));
%     disp(['Processing Subject ', currentErpFile]);
    ...
    eventType = nan(1,n_trials);
    eventType(events) = mark.conds;
    eventType(indices) = max(mark.conds)+1;
    latency=1:n_trials;

    if strcmp(sorting_trials,'Condition & Latency')
        ...
        if exist('eventType', 'var') && exist('latency', 'var')
            data = sort_trials(data, latency, eventType, sorting_trials, sort_idx);
        end
    end
    clear EEG;
    ...
    markFile = fullfile(outputDir, ['fileinfo_' num2str(nSub) '.mat']);
    ...
    icatb_save(markFile, 'eventType','n_trials','n_chans','n_samples');
...
function data = sort_trials(data, latency, eventType, sorting_trials, sort_idx)
...
if strcmpi(sorting_trials, 'condition & latency')
    ...
    uniqueCond = [sort_idx uniqueCond(~ismember(uniqueCond,sort_idx))];
