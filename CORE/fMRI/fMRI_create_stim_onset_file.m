...
studyname = 'CORE';
subnum = 'pilot';
dname = 'C:\Matlab_files\CORE\fMRI\SCIn_ouputs';
...
for i = 1:size(events,1)
    ...
    fid = fopen([studyname subnum '_' events{i,1} '_event.txt'], 'wt');
    fprintf(fid, '%.2f\n', onsets);
    fclose(fid);
end
...
for i = 1:size(blocks,1)
    ...
    fid = fopen([studyname subnum '_' blocks{i,1} '_block.txt'], 'wt');
    fprintf(fid, '%.2f\n', onsets);
    fclose(fid);
    
    fid = fopen([studyname subnum '_' blocks{i,1} '_block_dur.txt'], 'wt');
    fprintf(fid, '%.2f\n', durations);
    fclose(fid);
end