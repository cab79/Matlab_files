...
if ~exist(fullfile(S.outpath,erp_sname),'file') || ~S.use_data
    for f = 1:length(files_bal)
        ...
        base = mean(dw(:,basewin(1):basewin(2)-1),2);
        dw = dw - repmat(base,1,size(dw,2));

        % add to GD
        GD.giw(:,:,:,f) = iw;
        GD.gdw(:,:,f) = dw;
    end
    GD.chanlocs = EEG.chanlocs;
    GD.times = EEG.times;
    GD.S = S; % save settings for future reference
...
for f = 1:length(files_bal)
    ...
    % spatial mask
    spatmask = zeros(size(GD.gdw(:,:,f)));
    spatmask(elec,:) = 1; 
    
    % temporal mask
    tempmask = zeros(size(GD.gdw(:,:,f)));
    timewin=dsearchn(GD.times',S.timewin');
    ...
    nd_mask = length(find(erp_mask==1));
    temp = reshape((GD.gdw(:,:,f) .* erp_mask),1,[]);
    mask_erp = zeros(size(temp));
    [~,idx] = sort(temp(:),'descend');
    erp_mask2 = idx(1:nd_mask*S.mask_thresh);
    mask_erp(erp_mask2)=1;
    mask_erp = reshape(mask_erp,size(GD.gdw(:,:,f)));
