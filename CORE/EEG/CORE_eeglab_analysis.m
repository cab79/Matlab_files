...
S.path.raw = 'Q:\CORE\EEG\raw'; % unprocessed data in original or BIDS format
...
% S.import.fname.parts = {'subject','block','suffix','ext'}; % parts of the input filename separated by underscores, e.g.: {'prefix','study','group','subject','session','block','cond','suffix','ext'};
% S.import.fname.partsep = {'_',' ','',''}; % separator used in filename after each part (must be one per part even if empty)
% S.import.fname.ext = {''}; % file extension of input data: supports 'vhdr' (Brainvision), 'cnt' (Neuroscan), 'mff' (EGI - requires mffimport2.0 toolbox)
% S.import.study = {};
% S.import.select.groups = {}; % either single/multiple groups, or use * to process all groups in the datfile
% S.import.select.subjects = {'CORE041'}; % either single/multiple subjects, or leave blank to process all subjects in the datfile
% S.import.select.sessions = {}; % Can use * as wildcard
% S.import.select.blocks = {'4'}; % blocks to load (each a separate file). Can use * as wildcard
% S.import.select.conds = {}; % conditions to load (each a separate file). Can use * as wildcard
% S.import.select.chans = {[],[1,8,14,17,21,25,32,38,43,44,48,49,56,63,64,68,69,73,74,81,82,88,89,94,95,99,107,113,114,119,120,121,125,126,127,128]}; % include (first cell) or exclude (second cell) channels, or leave empty (default all chans)
...
% S.prep.fname.parts = {'subject','block','ext'}; % parts of the input filename separated by underscores, e.g.: {'prefix','study','group','subject','session','block','cond','suffix','ext'};
% S.prep.fname.ext = {'set'};
% S.prep.study = {};
% S.prep.select.groups = {}; % either single/multiple groups, or leave empty to process all groups in the datfile
% S.prep.select.subjects = {}; % either a single subject, or leave blank to process all subjects in folder
% S.prep.select.sessions = {};
% S.prep.select.blocks = {'2'}; % blocks to load (each a separate file) - empty means all of them, or not defined
...
% S.prep.cont.downsample = 0;
% S.prep.chan.addloc = 0; % add channel locations from this path; or leave as ''
% S.prep.chan.interp = [];
% S.prep.chan.reref = 2;
% S.prep.filter.notch = {[45 55],[95 105]};
% S.prep.filter.incl = [0.5 100];%[0.1 100]; % FILTER - INCLUSIVE
% S.prep.epoch.markers = {'DIN2'};
% S.prep.epoch.addmarker = 0; % set to 1 to add markers if the above are not present: will use the first marker value
% S.prep.epoch.timewin = [-0.2 0.9]; % peri-marker time window. 
% S.prep.epoch.detrend = 1;
% S.prep.epoch.rmbase = 1;
% S.prep.epoch.basewin = [-0.2 0]; % baseline window
% S.prep.startfile = 1; 
...
% S.prep.clean.FTrej = {[20 100]}; % high freq to identify noise, low freq to identify eye movement
% S.prep.startfile = 1; 
% S=eeglab_preprocess(S,'rej')
% save(fullfile(S.path.main,'S'),'S'); % saves 'S' - will be overwritten each time the script is run, so is just a temporary variable
% %% Separate files into left and right hand
% S.prep.separatefiles.on = 1;
% S.prep.separate.markerindex = {[1:4 9:12 17:20],[5:8 13:16 21:24]}; % index of markers to separate into files
% S.prep.separatefiles.suffix = {'left','right'}; % suffixes to new file names: one per cell of S.epoch.separate
% S=eeglab_preprocess(S,'sep')
% save(fullfile(S.path.main,'S'),'S'); % saves 'S' - will be overwritten each time the script is run, so is just a temporary variable
% %% STEP 4: ICA
% S.prep.load.suffix = {'sep'}; % suffix to add to input file name, if needed. Can use * as wildcard
...
S.path.prep = 'C:\Data\CORE\eeg\ana\prep\cleaned\part2'; % folder to load data from
...
S.tf.study = {'CORE'};
S.tf.load.suffix = {'2_merged_cleaned'}; % suffix to add to input file name, if needed. Can use * as wildcard
...
S.tf.select.datatype = 'TF'; %'Freq','TF','Coh','ERP'
S.tf.select.freq = [10,20,30,40]; % select frequency range and resolution (if freq/TF analysis). 1D array produces all freqs enteredl 2D array uses ranges, e.g. [0 4; 4 8; 8 13; 13 30; 30 40];
% general settings
S.tf.epoch.downsample = 125;
S.tf.epoch.detrend = 0;
S.tf.epoch.basewin = [-0.2 0]; % baseline window
S.tf.epoch.rmbase = 0; % remove baseline prior to frequency/ERP
S.tf.epoch.markers = {}; % marker types to analyse
S.tf.epoch.combinemarkers = 1; %include all events in a single condition.
% freq analysis settings
S.tf.freq.type = 'both'; % induced or both
S.tf.freq.basenorm = ''; % for TF analysis only: 'absolute', 'relative', 'relchange', 'normchange'
S.tf.freq.bootrep = 500;% Bootstraps repetitions (for coherence analysis only)
S.tf.freq.method = 'mtmconvol'; % options: 'mtmfft' (Freq),'mtmconvol' (TF), 'wavelet' (TF), 
S.tf.freq.param=6; % for mtmfft or wavelet, number of cycles. For multitaper, smoothness. If set to zero the time window will be fixed across frequencies to the smallest possible given the freq resolution.
S.tf.freq.taper = 'dpss'; % 'hanning' or 'dpss'
S.tf.freq.pad = 'min_pad';
S.tf.op.mov_avg_trials = 0; % N trials: average EEG power over trials using a moving average window
S.tf.op.mov_avg_trials_step = 0;
