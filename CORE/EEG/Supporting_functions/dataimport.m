function dataimport(filepath, basename, parts, combine, savepath)%,firstsample,lastsample)% use the very specific file name, and the whole of it!
...
for f = 1:length(parts)
    ...
    if length(mfffiles) > 1
        ...
        fprintf('\nProcessing %s.\n\n', filename);
        EEG = pop_readegimff(fullfile(filepath, filename));%,'firstsample',firstsample,'lastsample',lastsample);
    end
