function c = GBM_config_Bayesopt(S,varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if nargin>1
    failed = varargin{1};
else
    ...
    case 'binary'
        ...
        switch S.perc_modelspec.likelihood.inputvar
            case 'uncertain_unequal'
                c.like.logal0mu = repmat(log(0.01),1,c.n_inputcond);
                c.like.logal0sa = repmat(1,1,c.n_inputcond); % unfixed
                c.like.logal0var = true; % this is a variance parameter
                c.like.logal1mu = repmat(log(0.01),1,c.n_inputcond);
                c.like.logal1sa = repmat(1,1,c.n_inputcond); % unfixed
                c.like.logal1var = true; % this is a variance parameter
            case 'uncertain_equal'
                % only specify al0
                c.like.logal0mu = log(mult_al*[0.1 0.1 0.18 0.19]);%repmat(log(1),1,c.n_inputcond);
                c.like.logal0sa = repmat(var_al,1,c.n_inputcond); % unfixed
                c.like.logal0var = true; % this is a variance parameter
            case 'certain'
                % only specify al0
    ...
    switch S.perc_modelspec.priormodels.(type).priortype
        case 'hierarchical'
            ...
            if ~(strcmp(S.perc_modelspec.priormodels.(type).priorupdate,'static') && S.perc_modelspec.priormodels.(type).n_priorlevels==1)
                ...
                % Format: row vector of length n_levels.
                % Undefined (therefore NaN) at the first level.
                c.(type).ommu = [NaN, repmat(log(mult_om*exp(-5.75))-failed, 1, length(c.(type).mu_0mu)-2),log(mult_om*exp(-5.34))]; % BO 4 alpha
                %c.(type).ommu = [NaN, repmat(-5.16-S.failed, 1, length(c.(type).mu_0mu)-2),-5.58]; % BO 1 alpha
