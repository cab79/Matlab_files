function c = GBM_config_CORE_model(S,varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if nargin>1
    failed = varargin{1};
else
    ...
    case 'binary'
        ...
        switch S.perc_modelspec.likelihood.inputvar
            ...
            case 'uncertain_equal'
                % only specify al0
                c.like.logal0mu = log([0.0850,0.0937,0.1619,0.1483]);%repmat(log(1),1,c.n_inputcond);
    ...
    switch S.perc_modelspec.priormodels.(type).priortype
        case 'hierarchical'
            ...
            if ~(strcmp(S.perc_modelspec.priormodels.(type).priorupdate,'static') && S.perc_modelspec.priormodels.(type).n_priorlevels==1)
                ...
                % Format: row vector of length n_levels.
                % Undefined (therefore NaN) at the first level.
                c.(type).ommu = [NaN, repmat(-5.3572 -failed, 1, length(c.(type).mu_0mu)-2),-6.2149]; 
