...
% specify priors
%prior_file = 'CORE_fittedparameters_percmodel3_respmodel4_fractrain1_20190114T061324_CRPS_it14.mat';
prior_file = 'CORE_fittedparameters_percmodel3_respmodel4_fractrain1_20190114T061503_HC_it14.mat';
Sfile = load(fullfile(S.path.hgf,'fitted',prior_file),'S');
S.c_obs = Sfile.S.c_obs;
S.c_prc = Sfile.S.c_prc; 

% run
D_fit=HGF_run(D_train,S,0);
save(fullfile(S.path.hgf,'fitted',strrep(prior_file,'.mat','_priors_allsubs.mat')), 'D_fit', 'S');
...
if 0
    ...
    S.fname_ext= {
        '_fractrain1_20190114T061324_CRPS_it14_priors_allsubs.mat'
        '_fractrain1_20190114T061503_HC_it14_priors_allsubs.mat';
        };
    S.pep_flag = 1; % display PEP instead of EP
    S.family_on = 0;
    [~,~,out]=HGF_group_model_comparison(S);
end
