...
S.p_group = p_group;
S.ind = ind;
[ns,nrand] = make_input_files(D_fit,S,pth); 
...
function [ns,nrand] = make_input_files(D,S,pth)
...
for i = 1:nrand
    ...
    grp_ind = find(grps_rand(i,:)==grp)-1;
    for ns = 1:length(grp_ind)
        
        % input file index
        ii = nrand*(i-1) + (ns-1);
        
        % copy file
        src=fullfile(S.path.hgf,'input_files_orig',['input' num2str(grp_ind(ns)) '.mat']);
