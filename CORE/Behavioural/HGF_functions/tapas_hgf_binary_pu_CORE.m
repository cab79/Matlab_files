function [traj, infStates] = tapas_hgf_binary_pu_CORE(r, p, varargin)
...
if ~isempty(varargin) && strcmp(varargin{1},'trans');
    p = tapas_hgf_binary_pu_CORE_transp(r, p);
end
...
th   = exp(p(5*l-1));
al   = p(5*l:5*l+in-1);
eta0 = p(5*l+in:5*l+2*in-1);
eta1 = p(5*l+2*in:5*l+3*in-1);
...
for k = 2:1:n
    if not(ismember(k-1, r.ign))
        ...
        und1 = exp(-(u(k,1) -eta1(u(k,2)))^2/(2*al(u(k,2))));
        und0 = exp(-(u(k,1) -eta0(u(k,2)))^2/(2*al(u(k,2))));
