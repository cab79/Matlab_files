function [logp, yhat, res] = mismatch_response_epsi_binary(r, infStates, ptrans)
...
try % CAB
    logmm = be0 +be1.*surp +be2.*epsi1reg +be3.*epsi2reg +be4.*epsi3reg +be5.*da3reg;
catch
    if l<2
        logmm = be0 +be1.*surp +be2.*epsi1reg;
    elseif l<3
        logmm = be0 +be1.*surp +be2.*epsi1reg +be3.*epsi2reg;
    end
end
