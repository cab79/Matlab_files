...
if all(ismember({'FidNz', 'FidT9', 'FidT10'}, meegfid.fid.label))
    S =[];
    S.sourcefid = meegfid;
    S.targetfid = mrifid;
    ...
    %else
        M1 = [];
        S.sourcefid.fid.label{strmatch('FidNz', S.sourcefid.fid.label, 'exact')} = 'nas';
        S.sourcefid.fid.label{strmatch('FidT9', S.sourcefid.fid.label, 'exact')} = 'FIL_CTF_L';
        S.sourcefid.fid.label{strmatch('FidT10', S.sourcefid.fid.label, 'exact')} = 'FIL_CTF_R';
        S.targetfid.fid.pnt = S.targetfid.fid.pnt([1 4 5], :);
        S.targetfid.fid.label = S.targetfid.fid.label([1 4 5], :);
        S.useheadshape = 1;
    %end
elseif all(ismember({'spmnas', 'spmlpa', 'spmrpa'}, meegfid.fid.label)) && isempty(D.sensors('MEG'))
    S =[];
    S.sourcefid = meegfid;
    S.targetfid = mrifid;
