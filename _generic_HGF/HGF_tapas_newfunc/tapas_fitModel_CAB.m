function r = tapas_fitModel_CAB(responses, inputs, varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
try
    ...
    disp('UNABLE TO ESTIMATE MODEL EVIDENCE')
end
...
if any(isinf(H(:))) || any(isnan(H(:))) || any(eig(H)<=0)
    ...
    Sigma = inv(H);
    Sigma = (Sigma' + Sigma)./2;
    if any(isinf(Sigma(:))) || any(isnan(Sigma(:)))
        disp('Warning: Cannot calculate LME because the Hessian is not positive definite.')
        return
    else
        % Parameter correlation
        Corr = tapas_Cov2Corr(Sigma);
