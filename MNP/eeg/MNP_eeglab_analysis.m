...
S.import.fname.parts = {'study','group','subject','block','cond','ext'}; % parts of the input filename separated by underscores, e.g.: {'prefix','study','group','subject','session','block','cond','suffix','ext'};
S.import.fname.partsep = {'_','_','_','_','_','_'}; % separator used in filename after each part (must be one per part even if empty)
...
S.import.select.subjects = {'eegcab1'}; % either single/multiple subjects, or leave blank to process all subjects in the datfile
...
S.import.select.conds = {'AL'}; % conditions to load (each a separate file). Can use * as wildcard
S.import.select.chans = {[8 24],[]}; % include (first cell) or exclude (second cell) channels, or leave empty (default all chans)
...
S.prep.fname.parts = {'study','group','subject','block','cond','ext'}; % parts of the input filename separated by underscores, e.g.: {'prefix','study','group','subject','session','block','cond','suffix','ext'};
...
S.prep.select.subjects = {'eegcab1'}; % either single/multiple subjects, or leave blank to process all subjects in the datfile
...
load(fullfile(S.path.main,'S'))
S.tf=struct;% clears the field
S.path.prep = 'C:\Data\MNP\eeg\ana\prep\manrej'; % folder to load data from
S.tf.fname.parts = {'study','group','subject','block','cond','suffix','ext'}; % parts of the input filename separated by underscores, e.g.: {'prefix','study','group','subject','session','block','cond','suffix','ext'};
...
S.tf.select.subjects = {'eegcab1'}; % either single/multiple subjects, or leave blank to process all subjects in the datfile
...
% general settings
S.tf.epoch.basewin = [-0.5 0]; % baseline window
...
S.ploterp.fname.parts = {'study','group','subject','block','cond','suffix','ext'}; % parts of the input filename separated by underscores, e.g.: {'prefix','study','group','subject','session','block','cond','suffix','ext'};
S.ploterp.study = {'MNP'};
...
S.ploterp.select.subjects = {'eegcab1'}; % either a single subject, or leave blank to process all subjects in folder
...
S.ploterp.times= {[-0.5 0],[0 0.1],[0.1 0.2],[0.2 0.4]}; 
% RUN
S=plot_ERPs(S);
save(fullfile(S.path.main,'S'),'S'); % saves 'S' - will be overwritten each time the script is run, so is just a temporary variable
