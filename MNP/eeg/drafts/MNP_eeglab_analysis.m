...
S.prep.select.conds = {'AL'}; % conditions to load (each a separate file). Can use * as wildcard
...
S.prep.epoch.timewin = [-3 2]; % peri-marker time window. 
...
S.prep2.select.conds = {'AL'}; % conditions to load (each a separate file) - empty means all of them, or not defined
...
S.tf.select.conds = {'AL'}; % conditions to load (each a separate file). Can use * as wildcard
...
S.tf.epoch.markers = {'S  1','S  2','S  3','S  4','S  5','S  6'}; % marker types to analyse
S.tf.epoch.combinemarkers = 0; %include all events in a single condition.
% ERP settings
...
S.ploterp.select.conds = {'AL'}; % conditions to load (each a separate file) - empty means all of them, or not defined
S.ploterp.select.events = 1:6;
