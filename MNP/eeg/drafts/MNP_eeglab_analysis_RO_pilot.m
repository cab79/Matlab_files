...
S.import.fname.parts = {'study','subject','cond','ext'}; % parts of the input filename separated by underscores, e.g.: {'prefix','study','group','subject','session','block','cond','suffix','ext'};
...
S.import.select.subjects = {'pilot1'}; % either single/multiple subjects, or leave blank to process all subjects in the datfile
S.import.select.sessions = {}; % Can use * as wildcard
S.import.select.blocks = {}; % blocks to load (each a separate file). Can use * as wildcard
S.import.select.conds = {'AL','RO'}; % conditions to load (each a separate file). Can use * as wildcard
...
S.prep.fname.parts = {'study','subject','cond','suffix','ext'}; % parts of the input filename separated by underscores, e.g.: {'prefix','study','group','subject','session','block','cond','suffix','ext'};
S.prep.fname.partsep = {'_','_','_','_','_'}; % separator used in filename after each part (must be one per part even if empty)
...
S.prep.select.subjects = {'pilot1'}; % either single/multiple subjects, or leave blank to process all subjects in the datfile
S.prep.select.sessions = {}; % Can use * as wildcard
S.prep.select.blocks = {}; % blocks to load (each a separate file). Can use * as wildcard
S.prep.select.conds = {'RO'}; % conditions to load (each a separate file). Can use * as wildcard
...
S.prep.epoch.markers = {'S  1','S  2','S  3','S  4','S  5','S  6','S  7','S  8','S  9','S 10','S 11','S 12','S 13','S 14','S 15','R  1'};
S.prep.epoch.addmarker = 0; % set to 1 to add markers if the above are not present: will use the first marker value
S.prep.epoch.timewin = [-0.2 0.3]; % peri-marker time window. 
S.prep.epoch.detrend = 1;
S.prep.epoch.rmbase = 1;
S.prep.epoch.basewin = [-0.2 0.3]; % baseline window
S.prep.startfile = 1; 
...
S.prep.clean.flatchan.varthresh = 0; % std threshold - less variance than this per trial will be rejected
...
S.prep2.fname.parts = {'study','subject','cond','suffix','ext'}; % parts of the input filename separated by underscores, e.g.: {'prefix','study','group','subject','session','block','cond','suffix','ext'};
...
S.prep2.select.subjects = {'pilot1'}; % either a single subject, or leave blank to process all subjects in folder
S.prep2.select.sessions = {};
S.prep2.select.blocks = {}; % blocks to load (each a separate file) - empty means all of them, or not defined
S.prep2.select.conds = {'RO'}; % conditions to load (each a separate file) - empty means all of them, or not defined
...
S.tf.fname.parts = {'study','subject','cond','suffix','ext'}; % parts of the input filename separated by underscores, e.g.: {'prefix','study','group','subject','session','block','cond','suffix','ext'};
...
S.tf.select.subjects = {'pilot1'}; % either single/multiple subjects, or leave blank to process all subjects in the datfile
S.tf.select.sessions = {}; % Can use * as wildcard
S.tf.select.blocks = {'NLT'}; % blocks to load (each a separate file). Can use * as wildcard
S.tf.select.conds = {'RO'}; % conditions to load (each a separate file). Can use * as wildcard
S.tf.load.suffix = {'*'}; % suffix to add to input file name, if needed. Can use * as wildcard
S.tf.select.datatype = 'ERP'; %'Freq','TF','Coh','ERP'
% general settings
S.tf.epoch.basewin = [-0.2 0]; % baseline window
S.tf.epoch.rmbase = 1; % remove baseline prior to frequency/ERP
S.tf.epoch.markers = {'S  1','S  2','S  3','S  4','S  5','S  6','S  7','S  8','S  9','S 10','S 11','S 12','S 13','S 14','S 15','R  1'};
S.tf.epoch.combinemarkers = 0; %include all events in a single condition.
% ERP settings
...
S.ploterp.fname.parts = {'study','subject','cond','suffix','ext'}; % parts of the input filename separated by underscores, e.g.: {'prefix','study','group','subject','session','block','cond','suffix','ext'};
S.ploterp.study = {'MNP'};
...
S.ploterp.select.subjects = {'pilot1'}; % either a single subject, or leave blank to process all subjects in folder
...
S.ploterp.select.conds = {'RO'}; % conditions to load (each a separate file) - empty means all of them, or not defined
% S.ploterp.select.events = {[1 5 9 13],[2 6 10 14],[3 7 11 15],[4 8 12 16]}; % oddball vs standard
% S.ploterp.event_labels = {'stan','odd','stan','odd'};% oddball vs standard
% S.ploterp.select.events = {[1 4 9 12],[2 3 10 11],[5 8 13 16],[6 7 14 15]}; % stimtype (intensities)
% S.ploterp.event_labels = {'1','2','3','4'};% stimtype
S.ploterp.select.events = {[5 13],[6 14],[8 16],[7 15]}; % oddball vs standard for largest intensity difference only
S.ploterp.event_labels = {'stan','odd','stan','odd'}; % oddball vs standard for largest intensity difference only
...
S.ploterp.times= {[-0.2 0],[0 0.1],[0.1 0.2]}; 
% RUN
S=plot_ERPs(S);
save(fullfile(S.path.main,'S'),'S'); % saves 'S' - will be overwritten each time the script is run, so is just a temporary variable
