function r = tapas_simModel_CAB(inputs, prc_model, prc_struct, varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
%
% USAGE:
%     sim = tapas_simModel(inputs, prc_model, prc_struct, obs_model, obs_pvec)
% 
% INPUT ARGUMENTS:
...
%
%     prc_struct           Row vector of perceptual model parameter values (see the corresponding model's
%                        configuration file).
%
...
%% Observation model
if nargin > 3
    obs_config_fun = varargin{1};
    %if ~isempty(varargin{1}) % CAB
    
        % Remember observation model
        r.c_sim.obs_model = varargin{1};

        % Read configuration of observation model
        if isempty(strfind(r.c_sim.obs_model,'tapas'))
            r.c_obs = eval([r.c_sim.obs_model '(r,S)']); % CAB
        else
            r.c_obs = eval(r.c_sim.obs_model);
        end

        % Store observation parameters
        obs_pstruct = varargin{2};
        obs_namep_fun = str2func(strrep(r.c_sim.obs_model, 'config', 'namep'));
        [r.p_obs,obs_pvec] = obs_namep_fun(obs_pstruct,r.c_obs);
        r.p_obs.p = obs_pvec;

        % Get function handle to observation model
        obs_fun = str2func(strrep(r.c_sim.obs_model, 'config', 'sim'));
