...
for m=1:c.nModels
    ...
    try
        if r.c_prc.(type).irregular_intervals
            if size(u,2) > 1
                t = [0; r.u(:,end)];
            else
...
for k=2:1:n
    ...
    if not(ismember(k-1, r.ign))
        ...
        if strcmp(r.c_prc.(type).priorsource,'hierarchical')
            if strcmp(r.c_prc.(type).priortype,'fixed')
            ...
            elseif strcmp(r.c_prc.(type).priortype,'dynamic')
                % 2nd level prediction
        ...
        elseif strcmp(r.c_prc.(type).priorsource,'state')
        ...
        elseif strcmp(r.c_prc.(type).priorsource,'state') % Kalman
