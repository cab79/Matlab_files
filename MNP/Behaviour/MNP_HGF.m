...
%     u=u';
    ...
    if isempty(sim_param)
        ...
        y=nan(size(D(1).Sequence.signal,2),2);

        %choices
        if any(strcmp(S.resp_modelspec.responses,'Ch'))
            y(:,1) = D(1).Processed.presssignal; % BINARY response
            y(y(:,1)==1)=0;
            y(y(:,1)==2)=1;
        end
    ...
    elseif hgf
        ...
        elseif S.bayes_opt
            bopars = tapas_fitModel_CAB([], u(1:nst,:), prc_model, obs_model, opt_algo,S); %BAYES OPTIMAL
        elseif strfind(S.prc_config,'tapas')
            bopars = tapas_fitModel(y(1:nst,:), u(1:nst,:), prc_model, obs_model, opt_algo,S);
