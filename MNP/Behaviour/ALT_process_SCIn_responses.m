...
clear S
S.rawpath = 'C:\Data\MNP\Pilots\ALT\raw'; % unprocessed data in original format
S.anapath = 'C:\Data\MNP\Pilots\ALT\processed'; % folder to save processed .set data
S.fnameparts = {'subject','block','',''}; % parts of the input filename separated by underscores, e.g.: {'study','subject','session','block','cond'};
S.subjects = {'EL','DM'}; % either a single subject, or leave blank to process all subjects in folder
S.sessions = {};
%S.blocks = {'Sequence_ALT_OptionAdaptive'}; % blocks to load (each a separate file) - empty means all of them, or not defined
S.blocks = {'Sequence_ALT_OptionALT_classical'}; % blocks to load (each a separate file) - empty means all of them, or not defined
S.conds = {}; % conditions to load (each a separate file) - empty means all of them, or not defined
S.datfile = 'C:\Data\MNP\Pilots\ALT\Participant_Data.xlsx'; % .xlsx file to group participants; contains columns named 'Subject', 'Group', and any covariates of interest
