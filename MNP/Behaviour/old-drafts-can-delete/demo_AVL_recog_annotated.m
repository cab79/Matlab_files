% OTO: demo 'observing the observer': http://journals.plos.org/plosone/article?id=10.1371/journal.pone.0015555
% This simulates observations, and responses to those observations, under
...
% mu: class-specific 1st order moment (MEAN) of stimuli
...
n_t = 300; % number of trials ('k' in the paper)

% PAPER FIGURE 2: Level 1
switch flag
    % switches between different PERCEPTUAL MODELS of the subject's prior belief about
    % cue-outcome associations.
    
    case 1
        
        % assume static cue-outcome association
        % x2: hidden-states time series
        % sx2: measurement time series
        
        x2 = randn.*ones(1,n_t); % evolution function (static): prior probability of cue-outcome association. Random number from normal distribution.
        sx2 = sigm(x2); % observation function: transformation through sigmoid
        sx2 = sx2(:)'; % probability of the outcome given the prior (fixed over all trials)
       
        
    case 2
        
        % sample the cue-outcome association according to AR(1) model
        f_fname = @f_AR; % evolution function: gaussian random walk
        g_fname = @g_sigm; % observation function (sigmoid)
        [sx2,x2,x20,eta,e] = simulateNLSS(n_t,f_fname,g_fname,[],[],[],exp(-theta(2)),Inf,[],0);
        
    case 3 % this model is not in the paper
        
        % First sample the association volatility according to AR(1) model
        f_fname = @f_AR;
        g_fname = @g_Id; % identity: x3 and sx3 will be identical
        [~,x3,x30,eta,e] = simulateNLSS(n_t,f_fname,g_fname,[],[],[],exp(-theta(2)),Inf,[],0); % 
...
% PAPER FIGURE 2: Level 2
% Then randomly sample (ACTUAL) outcome identity from binomial pdf:
x1 = zeros(1,n_t);
seed = 1e4*rand;
for t=1:n_t
...
% PAPER FIGURE 2: Level 3
% Finally, sample visual (OBSERVED) outcome from Gaussian Mixture Model ...
u = zeros(2,n_t);
for t=1:n_t
    if x1(t)==1
...
% ADD IN A SMALL NUMBER OF RESPONSE ERRORS (EQUALLY SPACED)
...
[RT,x,x0,eta,e] = simulateNLSS(n_t,f_fname,g_fname,theta,phi,u,Inf,Inf,options,X0); % inversion algo
