function [L, ignore, ignore2, W, r,asize] = symmetric_orthogonalise_CAB(A, maintainMagnitudes)
...
if maintainMagnitudes,
	...
	if nargout > 1,
		% call function again
		[Lnorm, ~, ~, W,r,asize] = symmetric_orthogonalise_CAB(A * D, false);
		if isempty(Lnorm)
            L=Lnorm;
            return
        end
		...
		% call function again
		Lnorm = symmetric_orthogonalise_CAB(A * D, false);
    ...
    if ~isempty(S),
        ...
        if isFullRank,
            ...
            L=[];W=[];asize=size(A,2);
            return
        	error('ROInets:RankError',error_message,r,asize);
