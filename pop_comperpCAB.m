...
function [erp1, erp2, erpsub, times, pvalues, tvalues, erp1_sd, erp2_sd] = pop_comperpCAB( ALLEEG, flag, datadd, datsub, varargin);
...
if length(datsub) > 0 % dataset to subtract
    ...
    % -----------------------------
    if ~isempty(g.alpha)
        [pvalues tvalues] = pttest(erp1ind(g.chans,:,:), erp2ind(g.chans,:,:), 3);
...
erp1   = mean(erp1ind,3);
erp1_sd   = squeeze(std(permute(erp1ind,[3 1 2])));
if length(datsub) > 0 % dataset to subtract
    erp2   = mean(erp2ind,3);
    erp2_sd   = squeeze(std(permute(erp2ind,[3 1 2])));
    erpsub = erp1-erp2;
else
    erp2 = [];
    erpsub = [];
