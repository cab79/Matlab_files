...
cd(filepath);
select = 'ERP-'; TFmethod = '';
savenametype = '_FNUM_CNUM';
do_topo=0;
load([select savenametype '_data.mat'])

anatype = {'evoked', 'induced','itc'}; anaDAT=1; % 1:evoked, 2:induced, 3: ITC
singtrial=0;
handtype = {'LR', 'AU'}; hand_ana=1; % 1:LR, 2:AU
GFPnormtype = {'nonorm', 'GFPnorm', 'GFPpriornorm1','GFPpriornorm2','GFPbase','GFPp1norm'}; GFPnorm=1;%
avcond = {2,[4 6 8],10}; avcondname = 'midavnorep';% separate conditions with no repetition;
%avcond = {2,4,6,8,10}; avcondname = 'eachdigitnorep';% separate conditions with no repetition;
%avcond = {[1:10]}; avcondname = 'avdigit'; % average all conditions together
%avcond = {[1],[2]}; avcondname = 'eachcond'; % average all conditions together
%avcond = {[2:2:10]}; avcondname = 'avdigitnorep'; % average all conditions together
%avcond = {[2 10], [4 6 8]}; avcondname = 'outerinnernorep'; % outer vs inner digits only after a change in location
%avcond = {1,2,3,4,5}; avcondname = 'eachdigit'; % each individual condition
%avcond = {[1 5],[2 3 4]}; avcondname = 'outerinner'; % outer vs inner digits
...
for s = 1:length(subjects)
    gavg{s,1} = zeros(el,length(times),nf);
    gavg{s,2} = 0;
    for s2 = 1:length(subjects{s,1}) 
        for i = 1:no_cond
            if DATs{s,1}{s2,1}{i,4}>=min_trials_gavg
                gavg{s,1} = gavg{s,1} + DATs{s,1}{s2,1}{i,anaDAT};
                gavg{s,2} = gavg{s,2}+1;
            end
        end
    end
    gavg{s,1} = gavg{s,1}/gavg{s,2};
end
...
for s = 1:length(subjects)
    for f = 1:size(gavg{s,1},3)
        ...
        gavg{s,1}(:,:,f) = gavg{s,1}(:,:,f)/nanmean(std(gavg{s,1}(:,:,f),1));
    end
end
...
elseif strcmp(select,'TF')
    ...
    figure;
    imagesc(times,freqs,squeeze(mean(gavg_sm{s,1}([6 62],:,:),1))'); 
    colormap jet;
    set(gca,'YDir','normal')
end
    ...
    if no_peaks_temp>0
        ...
        for s = 1:length(subjects)
            for n = 1:no_peaks_temp
                TOPO(:,s,n) = mean(mean(gavg{s,1}(:,limits_all{f}{n},freq_idx{f}),3),2);
                %subplot(length(subjects),no_peaks,(s-1)*no_peaks+n); topoplot(TOPO(:,s,n), EEG.chanlocs);
            end
            TOPOSIM(s,:) = pdist(squeeze(TOPO(:,s,:))','cosine');
        end
        ...
        for s = 1:length(subjects)
            for n = 1:no_peaks
                subplot(length(subjects),no_peaks,(s-1)*no_peaks+n); topoplot(mean(mean(gavg{s,1}(:,limits{f}{n},freq_idx{f}),3),2), EEG.chanlocs);
            end
        end
    else
        no_peaks=0;
    ...
    for s = 1:size(sublist,2) 
        ...
        for f = 1:no_freqs
            ...
            for p = 1:no_peaks
                ...
                elseif hand_ana==1
                    hands = hand_nme;
                    hi = [ones(1,size(aff_side,2)); 2*ones(1,size(aff_side,2))];
                end
                for h = 1:length(hands)
                    h_name = hands{h};
                    top=[];
                    ...
                    for ac = 1:length(avcond)
                        ...
                        ntrials=[];
                        for i = 1:length(avcond{ac}) % for selected conditions
                            if ~isempty(DATs{(g-1)*2+hi(h,s),1}{s,1}{avcond{ac}(i),1})
                                DAT(:,:,:,i) = DATs{(g-1)*2+hi(h,s),1}{s,1}{avcond{ac}(i),1};
                                ntrials(i) = DATs{(g-1)*2+hi(h,s),1}{s,1}{avcond{ac}(i),4};
                            else
                                DAT(:,:,:,i) = NaN;
                            end
                        end
...
writetable(T_GFP,['gfp_results_' select '_' anatype{anaDAT} savenametype '_' handtype{hand_ana} '_' avcondname '_' GFPnormtype{GFPnorm} '.xlsx']);
