...
maxtrllength = length(data.time{1});%round((max(endsamplatency)-min(begsamplatency))*data.fsample) + 1;       % in samples
abstimvec    = data.time{1}; %((1:maxtrllength) + min(offset) -1)./data.fsample;                       % in seconds
