clear all;
cd('W:\Data\CRPS_Digit_Perception_exp1\Results\SPM sensor stats\Regression - indCond\flipped - Exp1-Exp2\Acc\subject effect');
load cov
clear fnames;
            ...
            for j = 1:no_peaks
                ...
                if use_flipped==1
                    if strfind(tmp_nme, 'right')
                        nme = dir(fullfile(filepath,tmp_nme,['type_' num2str(trials(i))],'strial*flip_reorient_Exp1-Exp2.img'));
                    else
                        nme = dir(fullfile(filepath,tmp_nme,['type_' num2str(trials(i))],'strial*Exp1-Exp2.img'));
                    end
                else
                    nme = dir(fullfile(filepath,tmp_nme,['type_' num2str(trials(i))],'strial*Exp1-Exp2.img'));
                end
