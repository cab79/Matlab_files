...
for f = 1:length(files)
   ...
   if combine_all == 1;
       ...
       correctRank = EEG.nbchan-1;
       EEG = pop_runica(EEG, 'extended',1,'interupt','on','pca',20);
       sname = [C{1} '_Exp1_left_ICA.set'];
       EEG = pop_saveset(EEG,'filename',sname,'filepath',raw_path); 
       trials = n1l_trials;
       save([C{1} '_Exp1_left_trials.mat'],'trials');
       
       EEG = pop_reref( EEGn1r, []);
       correctRank = EEG.nbchan-1;
       EEG = pop_runica(EEG, 'extended',1,'interupt','on','pca',20);
       sname = [C{1} '_Exp1_right_ICA.set'];
       EEG = pop_saveset(EEG,'filename',sname,'filepath',raw_path); 
       trials = n1r_trials;
       save([C{1} '_Exp1_right_trials.mat'],'trials');
       
       EEG = pop_reref( EEGn2l, []);
       correctRank = EEG.nbchan-1;
       EEG = pop_runica(EEG, 'extended',1,'interupt','on','pca',20);
       sname = [C{1} '_Exp2_left_ICA.set'];
       EEG = pop_saveset(EEG,'filename',sname,'filepath',raw_path); 
       trials = n2l_trials;
       save([C{1} '_Exp2_left_trials.mat'],'trials');
       
       EEG = pop_reref( EEGn2r, []);
       correctRank = EEG.nbchan-1;
       EEG = pop_runica(EEG, 'extended',1,'interupt','on','pca',20);
       sname = [C{1} '_Exp2_right_ICA.set'];
       EEG = pop_saveset(EEG,'filename',sname,'filepath',raw_path); 
       trials = n2r_trials;
       save([C{1} '_Exp2_right_trials.mat'],'trials');
   end
