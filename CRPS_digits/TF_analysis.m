...
DATs = subjects;
gavgL1 = zeros(el,190,23);
gavgR1 = zeros(el,190,23);
countavgL1 = 0;
countavgR1 = 0;
gavgL2 = zeros(el,190,23);
gavgR2 = zeros(el,190,23);
countavgL2 = 0;
countavgR2 = 0;
    ...
    for s2 = 1:length(subjects{s,1}) 
        ...
        for i = 1:no_cond
            ...
            for e = 1:size(EEG.data,1)
                [tfdata(:,:,e),x,y,times,freqs] = timef(EEG.data(e,:,find(fevents==cond(i))),[],[-200 800],EEG.srate,1,'detret','on','winsize',60,'plotersp','off','plotitc','off','plotphase','off');   
            end
            if strcmp(sublist_side{s},'L') && s==1
                ord = i;
                DATs{s,1}{s2,1}{ord,1} = permute(tfdata,[3 2 1]);
...
%RMSE = [];
%for s = 1:length(subjects)
%    for f = sort(1:size(gavg{s,1},3)-1,'descend')
%        for t = 1:size(gavg{s,1},2)
%            y = gavg{s,1}(:,t,f+1);%/std(gavg{s,1}(:,t,f+1));
%            yhat = gavg{s,1}(:,t,f);%/std(gavg{s,1}(:,t,f));
%            RMSE(s,f,t) = sqrt(mean((y - yhat).^2));
%        end
%    end
%end
%RMSEav = squeeze(mean(mean(RMSE,3),1));
%plot(freqs(1:end-1),RMSEav);
...
for s = 1:length(subjects)
    for e = 1:size(gavg{s,1},1)
        cormat(e,:,:,s) = corrcoef(squeeze(gavg{s,1}(e,:,:)));
        %pcamat(e,:,:,s) = pca(squeeze(gavg{s,1}(e,:,:)));
    end
end
cormatm = mean(cormat,4);
corav = squeeze(mean(cormatm,1));
%pcamatm = mean(pcamat,4);
%pcaav = squeeze(mean(pcamatm,1));

% Freq bin identification
no_freqs=6;
for threshcor=0.9:0.001:0.99;
...
freqs_limits = horzcat(freqs_limits,limits);
save freqs_limits freqs_limits
            ...
            for p = 1:no_peaks
                ...
                for h = 1:length(hand_nme)
                    ...
                    for ac = 1:length(avcond)
                        ...
                        top(:,ac) = mean(mean(DAT(:,find(times==lim(1)):find(times==lim(2)),freq_idx{f}),3),2);
...
writetable(T_GFP,'gfp_results_tf.xlsx');
writetable(T_LAT,'lat_results_tf.xlsx');
writetable(T_POW,'pow_results_tf.xlsx');
...
for f = 1:no_freqs
    ...
    for p = 1:no_peaks
        ...
        for h = 1:length(hand_nme)
            h_name = hand_nme{h};
            fieldname = [f_name '_' p_name '_' h_name]
            for n = 1:size(TOPdist,1)
                topmat(n,:) = TOPdist(n).(fieldname);
            end
            squareform(mean(topmat,1))
        end
    end
end
